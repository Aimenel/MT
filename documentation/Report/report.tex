...
In Carbon, the permission held for a predicate instance is stored in the map \textit{Mask}. An predicate location is  of the predicate \textit{pred} with n arguments is represented as \textit{Mask}\([null, pred(arg_1, \dots, arg_n)]\). . Similarly for the approach for quantified predicate permissions, we do not need to change anything in the current implementation, only extend it with a translation for inhaling and exhaling quantified predicate permissions. To translate a quantified predicate permission, we can reuse the solution presented for quantified fields, replacing the field locations with predicate locations. Additionally, we replace the neutral field location \(r\) with a set of stand-in arguments.
