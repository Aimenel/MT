...
Since field permissions are represented using Maps, it is sufficient to introduce a new translation for inhaling and exhaling quantified field permissions.
...
\begin{longtable}{| p{0.9\textwidth} |}
\hline
\textbf{inhale forall } \(x:T :: c(x) \Rightarrow  acc(pred(e_1 (x),\dots,e_n (x)), p(x)) \) \{\\
\ident \textit{/* Inverse Assumptions */} \\
\ident Let  \(e^{-1}\)  be a fresh function of type  \(T \rightarrow \) \textit{Ref} \\
\ident \textbf{assume\ } \( \forall r:Ref :: c(e^{-1}(r))  \Rightarrow \underline{e}(e^{-1}(r)) = r \) \\
\ident \textbf{assume } \( \forall x:T :: \underline{c}(x)  \Rightarrow e^{-1}(\underline{e}(x)) = x \) \\
\\
\ident \textit{/* assume permission is positive */} \\
\ident \textbf{assume } \( \forall x:T  \{\}:: \underline{c}(x)  \Rightarrow \underline{p}(x) >= 0 \) \\
\\
...
\ident \textbf{assume\ } \(\forall r:Ref :: \{tr\} c(inv(r)) \Rightarrow perm(r) = \) \textbf{old} \((perm(r))  + p(inv(r))\)\\
\\
\ident \textit{/* define independent permissions */} \\
\ident \textbf{assume\ } \(\forall r:Ref :: \{tr\}\ \neg c(inv(r)) \Rightarrow perm(r) = \) \textbf{old} \((perm(r)) \)\\
\ident \textbf{assume\ } \(\forall o:Obj, f_o:Field :: \{tr\}\ (f_o != f) \Rightarrow perm(o.f) = \) \textbf{old} \((perm(o.f)) \)\\
\}\\ \hline
