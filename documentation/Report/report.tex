...
The first problem is handled in section \ref{shr}, the second and third in section \ref{qpp}. the fourth question is answered  in section \ref{qppVals}. 
...
In order to translate the given permission to that form, we need to adapt the definition of the inverse function presented in section \ref{qf}. As before, the inverse function is used to get from a general location to the variable used in the original quantified permission expression. Before, that location was expressed by the neutral location \(r\). In this case, the general location is expressed by the predicate name and its arguments, \textit{pred}\((arg_1, \dots, arg_n)\). As soon as a single argument of two predicate location differs, it is not the same location.
...
Both carbon and silicon currently do not allow merging of pure and permission-based quantifiers, nor does it allow combinations of multiple quantified field permissions. Of course, this can be circumvented by writing multiple quantifiers by repeating the definition of the variables and conditions.  
To decrease the code size for specifying the properties desired and support user-friendliness for mixing quantifiers in both applications, we wished to further allow the following combinations of pure and permission-based quantifiers:
