...
\begin{tabular}{ l}
\textbf{Inhaling a Quantified Field Permission:}\\  \hline
inhale( \(h_0, \pi_0,  \mathbf{forall} x:T :: c(x) \Rightarrow acc(e(x).f, p(x))\) \{\\
\t test  \\
\}\\
\end{tabular}
...
\raggedright
\begin{flalign*}
\mathbf{Inhaling a Quantified Field Permission:} && \\
inhale(h_0, \pi_0,  \mathbf{forall} x:T :: c(x) \Rightarrow acc(e(x).f, p(x)) \{ && \\
\textrm{Let y be a fresh symbolic constant of Type T}\\
\textrm{\textit{/* Symbolically evaluate source-level expressions */}}\\
\mathbf{var} (\pi_1, \underline{c}(y)) := eval(h_0, π_0, c_y))\\
\mathbf{var} (\pi_2,\underline{e}(y)) := eval(h_0, π_0, e_y))\\
\mathbf{var} (\pi_3,\underline{p}(y)) := eval(h_0, π_0, p_y))\\
\mathbf{var} \pi_4 := \pi_3{\c(y)}\\
\\
\textrm{\textit{/*Introduce inverse function */}}\\
\textrm{Let } e^{-1} \textrm{  be a fresh function of type } T \rightarrow Ref\\
\mathbf{var} \pi_5 :=  \pi_4 \cup \{\forall r: Ref \cdot c(e^{-1}(r))  \Rightarrow \underline{e}(e^{-1}(r)) = r \} \\
\mathbf{var} \pi_6 :=  \pi_5 \cup \{\forall x:T \cdot c(e(x))  \Rightarrow e^{-1}(\underline{e}(r)) = x \} \\
\\
\textrm{Let } v \textrm{  be a fresh value map}\\
\mathbf{var} h_1 :=  h_0 \cup \{r.f \mapsto [v(r), \underline{c}(e^{-1}(r)) ? \underline{p}(e^{-1}(r)) : 0] \} \\
\mathbf{return} (h_1, \pi_6)\\
\}\\
\end{flalign*} 
\end{minipage}
%Inhaling Field
...
\begin{minipage}{3in}
\begin{flalign*}
\mathbf{Exhaling a Quantified Field Permission:}\\
exhale(h_0, \pi_0,  \mathbf{forall} x:T :: c(x) \Rightarrow acc(e(x).f, p(x)) \{ \\
\textrm{Let y be a fresh symbolic constant of Type T}\\
\textrm{\textit{/* Symbolically evaluate source-level expressions (as above)*/}}\\
[\dots]\\
\textrm{\textit{/*Check injectivity of receiver expression*/}}\\
\textrm{Let } y_1, y_2 \textrm{ be a fresh symbolic constants of Type T}\\
\mathbf{check} \pi_4 \models \underline{c}(y_1) \land \underline{c}(y_2) \land \underline{e}(y_1) = \underline{e}(y_2) \Rightarrow  y_1 = y_2\\
\\
\textrm{\textit{/*Introduce inverse function (as above*/}}\\
[\dots]\\
\textrm{\textit{/*Remove Permissions*/}}\\
\mathbf{var} h_1 :=  remove(h_0, \pi_6, f, (\lambda r \cdot \underline{c}(e^{-1}(r)) ? \underline{p}(e^{-1}(r)) : 0)) \\
\mathbf{return} (h_1, \pi_6)\\    
\}\\
\end{flalign*} 
\end{minipage}
