...
The second example, which is also included in the test run as \textit{two\_block\_array.sil}, describes  an array, which is accessed by predicates. The predicate \(access\) takes an array and an index of that array. If the index is an even number, it grants access to the array location of that index as well as the next one. The method \textit{totalSum} calculates the sum of all elements from the given even index \(i\) to the end of the array. The \textit{setVal} method sets the value of all entries of the given array from index \(i\) until the end, two at a time. The function \textit{setVal2} sets a new value to all entries from index \(i\) until the next write would be over the bounds of the array. 
\\
\begin{lstlisting}
