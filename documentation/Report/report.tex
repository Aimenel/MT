...
Silver currently does not allow merging of pure and permission-based quantifiers, nor does it allow combinations of multiple quantified permissions. This can be circumvented by writing multiple quantifiers by repeating the definition of the variables and conditions.
To decrease the code size for specifying the properties desired and support user-friendliness it would however be desirable to lift some of these restricitons.

We consider the support of the following expressions:
\begin{equation}
\label{eq1}
...
\end{equation}
where {\it trs} is a set of triggers, {\it c(x)} is  a boolean expression,{\it e(x)} denotes an expression, either pure or permission-based, which was already supported inside quantifier before.  If {\it e(x)} is indeed a permission expression, it consists of an injective mapping to a set of locations, where {\it p(x)} denotes a permission function as described in the chapters before.

The first expression allows to combine two expressions on the right-hand side of the implication. The second expression  grants us the ability to refine a condition.

\subsection{Conjunction}
A conjunction of two expressions is currently supported if the quantifier is pure only. As a first solution we propose to adapt the symbolic execution rules to handle these cases.
If the expression is fully pure, this is handled as previously.

The expressions of the combinations may be depending on each other, which is why they have to be evaluated sequentially. 
If both expressions denote a permission-based expression, the injectivity and inverse function need to be defined per expression. Besides the evaluation of the condition, each expression is handled independently.

As a second solution, we therefore propose to rewrite the given quantifiers to two quantifiers, which allows us to reuse the symbolic execution rules used before: 
\begin{equation}
\begin{aligned}
...
The triggers are passed along to both quantifiers.
...
\begin{itemize}
\item conjunction of field and predicate epxressions, as well as pure assertions:
...
\item adding additional conditions by implication:
%TODO: link?
\begin{equation}
	\mathbf{forall} \; x:T ::  \{trs\}\ c_1(x) \Rightarrow (c_2(x) \Rightarrow e(x)),
\end{equation}
...
We considered two ways to expand the given design to this setting: 
\begin{itemize}
\item As a first solution, we could adapt the rules for inhaling and exhaling permission-based quantifiers to handle such cases.
\item desugaring these forall expressions to the form we obtain a set of quantified field permissions, quantified predicate permissions and pure quantifiers.
\end{itemize}
 As a result there is barely any gain in not desugaring the terms mentioned before. Therefore we suggest two rewriting rules to desugare conjunctions and implications within a forall expression.

\subsection{Rewriting Rules}
\label{rewriting}
A conjunction of two expression is desugared by rewriting the entire forall expreeion into two forall expressions, duplicating variables, condition and triggers. Triggers are expected to be valid for both newly generated forall expressions.
...
The rewriting rules are implemented in the parser. As a result they are executed for both Carbon and Silicon.
