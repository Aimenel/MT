...
\section{Abstract}
Viper supports a quantified field permission of the form \(\mathbf{forall\ } x:T :: c(x) \Rightarrow acc(e(x).f, p(x))\). The quantifier may only be depending on one variable and express field a field permission. Viper consists of two verifiers. Carbon is based on condition generation, whereas Silicon is based symbolic execution. In this report, we present the approach for both types of verifiers to generalise this expression allowed to support predicate and magic wands. We also present rewriting rules for combining pure and permission-based quantifiers, as well as the adaption necessary to these approaches in order to support nested quantifiers.
...
\label{qf}
The implementation for quantified field permissions is depending on the verifier used. We first summarize how quantified field permissions are supported in a verification generator such as Carbon, and explain the approach for symbolic execution, as presented in the paper \textit{Automatic Verification of Iterated Separating Conjunctions using Symbolic Execution}\cite{isc}. 

The supported quantified field permission expression is defined as:
...
\subsubsection{Condition Generation and Quantified Field Permissions}
\label{cgqf}
In condition generation approaches, fields are
...
Inhaling a field permission \(r.f\) is translated an updated permission for the specified location \(Mask(r, f)\), as well as an non-null assumption for the reference \(r\). 
In order to frame the quantifier \(\mathbf{forall\ } x:T :: c(x) \Rightarrow acc(e(x).f, p(x))\), we create a new map and define the adapted permissions for the inhaled field permissions, as well as the permission which remain unchanged. In order to trigger these permissions for all read or write expressions later on, it is necessary to express the new permissions for a neutral location. A neutral field location is defined as \(r.f \), where \(r\) is a reference, and \(f\) a field.
In order to express the locations updated by the quantifier, we introduce a inverse function \(e_{-1}\), which maps the locations to the corresponding value x of type \(T\). Using that quantifier we can check, whether a location is altered b the quantifier by evaluating the condition as \(c(e_{-1}(r)\). The gained permission is equal to \(p(e_{-1}(r)\). 
The introduction enforces, that the expression \(e(x)\) should be injective. When inhaling a quantified field permission, the injectivity of \(e(x)\) is assumed along with the property, that all references affected by the quantifier are non-null.
...
\caption[carbon quantified field inhale]
   {Translating quantified field inhale operation} %expand description: check, remove....
\label{qfconditionInhale}
\end{longtable}

\paragraph{Exhaling a Quantified Field Permission: }
Exhaling a quantified field permission \(\mathbf{forall\ } x:T :: c(x) \Rightarrow acc(e(x).f, p(x))\), includes two checks. First, we assert that the expression \(e(x)\) is injective. Then, we check that we possess the permission needed for exhaling the permission for all locations. If both are true, we again introduce the inverse function and express the permissions of the map as when inhaling. 
...
\caption[carbon quantified field exhale]
   {Translating quantified field exhale operation} %expand description: check, remove....
\label{qfconditionExhale}
\end{longtable}


\subsubsection{Symbolic Execution Rules for Quantified Field Permissions}
The solution presented in  \textit{Automatic Verification of Iterated Separating Conjunctions using Symbolic Execution}\cite{isc} introduces a new type fo heap chunks, new rules for inhale and exhale single and quantified field permissions, as well as for reading and writing these field values.

%Quantified Field Chunk
\paragraph{Quantified Field Chunk:}
An field heap chunk \((r, f) \mapsto [v, p]\) is a defined for a reference and a field and holds the its value and its permission amount \(p\). \(r\) denotes a specific reference, \(f\) is a field defined in the program. The value \(v\) has the same type as the field \(f\), whereas \(p\) is a permission. 

To support quantified field chunks, a new type of chunk needs to defined. This chunk potentially holds an unbounded set of references and values. A straightforward implementation of the expression \(\mathbf{forall\ } x:T :: c(x) \Rightarrow acc(e(x).f, p(x))\) would result in an heap of the form  \((x, f) \mapsto [v(x), c(x) ? p(x) : 0)]\). Both value and permission are depending on x. The value \(v\) is changed to a value map storing the known values of the set. In order to check the permission amount of a field location, this would however result in a existential query over all quantified field chunks.

To avoid this, we change the representation to express the permission and values for a general location \(r\) of type Ref.  To translate the initial permission expression to that form, we define an inverse function, which maps a location to the value of the corresponding quantifier variable of Type T. The new definition for a quantified field chunk is \(r.f \mapsto [v(r), c(inv(r)) ? p(inv(r)) : 0)]\), where r is no longer a previously defined identifier, but the quantified receiver. The permission amount \( p(r)\) and value \(v(r)\) is expressed depending on that location.

To guarantee the existence of an inverse function, it is necessary that the expression \(e\) must be injective. This does not restrict the data structures that can be handled by this approach as long as correctly specified.
...
Inhaling a quantified field expression generates a new quantified heap chunk. In order to do that, we first evaluate the condition, receiver expression and permission expression and introduce the inverse function of the expression \(e(x)\). As a next step, we define the inverse function, which is only partially defined for the values where the given condition \(c(x)\) applies. The definition is expressed as two conditions which are added to the path conditions. As a last step, we generate the new quantified heap chunk using the previously defined inverse function  and add it to the heap. The value map is initially empty.
...
\caption[Exhaling a Quantified Field Permission]
   {These symbolic execution rules for exhaling a quantified field permission correspond to the rules defined in figure 2 of the paper \textit{Automatic Verification of Iterated Separating Conjunctions using Symbolic Execution}\cite{isc}. The \textbf{check} instruction is translated to a query, which is sent to the SMT solver for verification.}
\label{qfExhale}
\end{longtable}

Removing the permission returns an error, if not enough permission is held, or an updated heap with the removed permission otherwise. Since we create a new quantified field chunk for every inhale, the permission required may be held in multiple chunks simultaneously. When removing a certain permission amount, we first filter all quantified chunks with the required field \(f\). 
We proceed to remove permissions greedily until we removed the required permission amount. Updating chunks is expressed via point-wise defined functions defining the permission amount subtracted. After updating the permission for all quantified field chunk of field \(f\), we check that sufficient permission was taken and return the updated heap.
...
Due to the fact that a permission to a specific field may also include permission previously inhaled without a quantifier, single field chunks are no longer used for a quantified field. Instead, we represent such a field chunk as a quantified field chunk. To express the field permission \(acc(e.f, p)\)  as its equivalent quantified chunk, we define its permission as \(p\) if  the reference is equal to the location specified by the expression \(e\). The updated inhaling rules are presented in table \ref{sqfInhale}.
...
Exhaling a single field permission \(acc(e.f, p)\) is handled similarly. After evaluating the parameters, we call remove with, expressing the permission for the field location defined by the original epxression \(e\). The updated inhaling rules are presented in table \ref{sqfExhale}.
...
The value information of a quantified field chunk is stored in its value map \(v\). It represents a under-specified total function and since the field is already defined by the heap chunk, is defined per reference

%domain?
A symbolic heap may consist of multiple chunks for the same field. The domains of the value maps may overlap such that the "value of one location \(x.f\) may be represented by multiple maps when the receiver \(x\) is quantified over and the permissions to different instantiations of the qunatifier are recorded in different chunks. Therefore, all of these value maps need to be considered when evaluating a field access."

To provide a simple translation for field-lookups, we define a function which summarises all value maps of a field \(f\) lazily before evaluating an expression \(e.f\), as well as the permission expressions held per location. Path conditions are not considered.

"A symbolic heap may contain fractional permissions, 
...
\caption[Summarise Quantified Field Permissions]
   {These symbolic execution rules for summarizing quantified field permissions correspond to figure 5 of the paper \textit{Automatic Verification of Iterated Separating Conjunctions using Symbolic Execution}\cite{isc}. It introduces a fresh value map and walks through all quantified field chunks of field \(f\), summarizing the values for the defined locations and the permission for all references.}
\end{longtable}



\paragraph{Field Read}
inhaling and exhaling pure boolean expressions:  evaluating expression and either adding the resulting symbolic execution to the path condition or checking it (Append A)
...
\}\\ 
\\
\textbf{eval}\(\ (h_0, \pi_0, \mathbf{forall} x :: e(x)) \) \{\\
\ident Let \(y\) be a fresh symbolic constant
\ident \textbf{var } \((\pi_1, \underline{e}(y)) :=\) \textbf{eval} \(h_0, \pi_0, e(y))\) \\
\ident \textbf{return } \((\{  b \in \pi_1 | y  \notin FV(b)\} \cup \{ \forall x \cdot (\bigwedge_{ b \in \pi_1 | y  \notin FV(b)}  b[x / y])\}, \forall x \cdot \underline{c}(x))\) \\
\}\\ 
\hline
\caption[Summarise Quantified Field Permissions]
   {These symbolic execution rules for summarizing quantified field permissions correspond to figure 6 of the paper \textit{Automatic Verification of Iterated Separating Conjunctions using Symbolic Execution}\cite{isc}. It introduces a fresh value map and walks through all quantified field chunks of field \(f\), summarizing the values for the defined locations and the permission for all references.}
\end{longtable}



\paragraph{Field Write}
%field write: exhale acc(e_1.f)< inhale acc(e_2.f)<


\paragraph{Framing}

\paragraph{Controlling Quantifier Instantiations}
%TODO
