...
We consider to the support of the following expressions:
\begin{longtable}{ p{1\textwidth} } 
\textbf{forall} \(x:T :: \{trs\}\  Exp\),\\
\ident \( Exp := b\)  \\
\ident \ident \(|\ acc(e.f, p)\) \\
\ident \ident \(|\ acc(pred(arg_1, \dots, arg_n), p)\) \\
\ident \ident \(|\ A \wand B\) \\
\ident \ident \(|\ Exp_1 \ \&\&\ Exp_2\) \\
\ident \ident \(|\ b \Rightarrow Exp\), \\
\label{combinations}
\end{longtable}
where {\it trs} is a set of triggers and \(b\) a pure expression. The last two expressions allow us to combine pure and quantified expressions. For example, the following expression  is allowed: 
\begin{longtable}{ p{1\textwidth} } 
\textbf{forall} \(x:T :: x\ in\ xs \Rightarrow (i > 0 \Rightarrow (acc(x.f) \ \&\&\ x.f  = i)) \ \&\&\ (i <= 0 \Rightarrow acc(x.g)) \) \\
\end{longtable}

Other combinations using Boolean expressions are currently not considered unless they are part of a Boolean expression. This is due to the fact, that Viper does not supported them permission-based expressions as well. Combinations of pure quantifiers are already supported in Viper, as are quantified fields. The approaches for quantified predicates and magic wands are treated using the approaches defined as before. We treat the pure quantifiers and quantified permissions for fields, predicates and magic wands as our base case. In order to support the combinations defined above, we add three rewriting rules explained in the following sections.

If a quantified condition does not include any condition, the formula is rewritten to the form:
\begin{longtable}{ p{1\textwidth} } 
\textbf{forall} \(x:T :: true \Rightarrow Exp\) \\
\end{longtable}

In order to illustrate some of the challenges supporting these new expression, we discuss the following example:
\begin{longtable}{ p{1\textwidth} } 
\textbf{forall}  \(x:Ref :: acc(x.f) \&\& (i > 0 \Rightarrow (x.f = i)))\)\\
\end{longtable}
%dependency
...
\begin{equation}
\label{eq1}
	\mathbf{forall} \; x:Ref :: \{\}\  x\ \mathrm{in \ Set} \{\dots\} \Rightarrow acc(x.f) \ \&\& \ (x.f > 0)
\end{equation}
