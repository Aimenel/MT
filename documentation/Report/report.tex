...
\caption[carbon quantified field exhale]
   {Translating quantified field exhale operation. The trigger in line ? is added in the adapted translation, which is defined in table \ref{qfconditionExhaleExtended}.} %expand description: check, remove....
...
If a reference \(x\) is within the set of references \(xs\), we inhale the access to the field location defined by that reference and gain the information that its value is equal to \(i\). If a reference is within the set \(ys\), we gain the access right to the predicate \(P(x)\). Please not, that we cannot assume the value of a field without holding partial permissions. The sub-expressions of the combinations may depend on each other, which enforces that \(acc(x.f)\) needs to be inhaled before accessing the field \(x.f\).
...
The expression \(e(x_1,  \dots, x_n)\) stands for any unquantified permission where we presented a quantified solution in the previous chapters. Pure quantifiers of this form are already supported in Viper. For the support of this new quantified expression we present a new generalisation of the previously shown algorithms.\\

The second structure represents a truly nested quantifier, where a second quantified expression occurs on the right-hand side of the implication. The following formula represents such a form with two quantifiers:\\
...
As an additional change to the approaches, any occurrence of the inverse function is replaced by a tuple of all inverse function defined. We display the altered approach for inhaling a nested quantified field permission for verification condition generation in table \ref{qfnInhaleCond} and for symbolic execution in table \ref{qpnInhaleSyn}. Other approaches are adapted analogously for nested quantifiers, but are omitted here.
 

\subsection{Rewriting Rules for Nested Quantified Permissions}
\label{rewritesN}
If an expression is truly nested, we rewrite that expression to the newly established base cases by adding the new variables

\begin{longtable}{ p{1\textwidth} } 
      \(\mathbf{forall \ }  x_1:T_1, \dots x_n:T_n ::  \{trs_1\}\  c_1(x_1, \dots, x_n) \Rightarrow \) \\
\ident \ident \ident     \( ( \mathbf{forall \ } y_1:T_{n+1}, \dots, y_m: T_{n+m}:: \{trs_2\} \ c_2(x_1, \dots, x_n, y_1, \dots, y_{m}) \Rightarrow e(x_1, \dots, x_n, y_1, \dots, y_{m}))\)
\\
\( \longmapsto \mathbf{forall \ }  x_1:T_1, \dots x_n:T_{n}, y_1: T_{n+1}, \dots, y_m: T_{n+m} ::  \{trs_1 \times trs_2\}\ \) \\ 
\ident \ident \ident \(( c_1(x_1, \dots, x_n) \:\&\&\:   \ c_2(x_1, \dots, x_n, y_1, \dots, y_m) )  \Rightarrow  e(x_1, \dots, x_n, y_1, \dots, y_m)) \) \\
\end{longtable}
