...
"Dedicated assertions denote the permission to a heap location e.f, written as e.f -> in separation logic and as the accessibility predicate acc(e.f) in implicit dynamic frames; we use the latter in this paper."

Permission logics are designed to define which locations of a heap may or may not be accessed. 

Viper works with fractional permissions, where full permission denotes write and read access, permission greater than zero read access and zero permission no permission to read or write on that heap location. Assertions may only be rely on locations where sufficient permission are held.

"As soon as permission to a heap location is no longer held, information about its value cannot be retained."

"Inhaling an assertion A adds the permissions denoted by A to the current states and assumes the pure assertion A"
"Exhaling an assertion A checks that the state contains the permissions denoted by A and removes them."

Smallfoot-style Symbolic Exectuion:
symbolic state: heap chunks and path conditions
permission amount between 0 and 1
Checking permissions: iterating through the heap chunks

In successor tools: VeriFast, jSTar and Viper: heap chunks also predicates...
...
Injectivity due to separation logic: separating conjunction * expresses that permission denoted by the two conjuncts must be disjoint.
